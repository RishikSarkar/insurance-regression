...
def find_deriv_w(weights, bias, j):
    return np.sum(np.dot(np.sum(np.dot(x, weights) + bias - y), x[:, j])) / len(x)
...
weight, b = gradient_descent(w, 0, 0.001, 1000)
print("w: ", weight, "b: ", b)
...
age = float(input("Age: "))
sex = float(input("Sex (1/2): "))
bmi = float(input("BMI: "))
children = float(input("# Children: "))
smoker = float(input("Smoker (0/1): "))
region = float(input("Region (1/2/3/4 = SW/SE/NW/NE): "))
